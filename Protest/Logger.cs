...
internal static class Logger {
...
#if DEBUG
    public static void Error(in Exception ex, [CallerLineNumber] int line = 0, [CallerMemberName] string caller = null, [CallerFilePath] string file = null) {
...
#else
    public static void Error(in Exception ex) {
        Error(ex.Message);
    }
#endif

    public static void Error(in string ex) {
        lock (syncLog)
            try {
    ...
    public static void Action(in string initiator, in string action) {
