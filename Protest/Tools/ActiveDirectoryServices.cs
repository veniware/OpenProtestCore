...
internal static class ActiveDirectoryServices {
    ...
    public static Hashtable AdFetch(in SearchResult result) {
        ...
        ContentBuilderAddValue(result, "userPrincipalName", "USERNAME", hash, GetUsername);
    ...
    public static byte[] UnlockUser(in string[] para) {
        ...
        if (username.Length == 0 && Database.users.ContainsKey(filename)) {
            Database.Entry entry = (Database.Entry)Database.users[filename];
    ...
    public static byte[] DisableUser(in string[] para) {
        ...
        if (username.Length == 0 && Database.users.ContainsKey(filename)) {
            Database.Entry entry = (Database.Entry)Database.users[filename];
            if (entry.attributes.ContainsKey("USERNAME")) username = ((string[])entry.attributes["USERNAME"])[0];
        }

        if (username.Length == 0) return Strings.CODE_INF.Array;

        SearchResult sr = GetUser(username);
        if (sr is null) return Strings.CODE_INV.Array;
    ...
    public static byte[] EnableUser(in string[] para) {
        ...
        if (username.Length == 0 && Database.users.ContainsKey(filename)) {
            Database.Entry entry = Database.users[filename];
            if (entry.hash.ContainsKey("USERNAME")) username = ((string[])entry.hash["USERNAME"])[0];
        }
